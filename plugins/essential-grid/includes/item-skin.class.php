...
class Essential_Grid_Item_Skin {
    ...
    public function output_item_skin($demo = false, $choosen_skin = 0){
		...
		if($is_post) $li_class .= ' eg-post-id-'.@$this->post['ID'];
					...
					if(isset($this->media_sources[$order]) && $this->media_sources[$order] !== '' && $this->media_sources[$order] !== false){ //found entry
						...
						switch($order){
							...
							case 'content-wistia':
								...
								$echo_media = '<div class="esg-media-video" data-wistia="'.$this->media_sources[$order].'" width="'.$this->video_sizes[$this->video_ratios['wistia']]['width'].'" height="'.$this->video_sizes[$this->video_ratios['wistia']]['height'].'" data-poster="'.$video_poster_src.'"></div>';
