...
class Essential_Grid_Navigation {
	...
	public function set_show_count($opt, $key = ''){
		...
		$this->filter_show_count['filter'.$key] = $opt;
    ...
    public function set_filter_start_select($select){
		$this->filter_start_select = explode(',', $select);
    ...
    public function output_filter($demo = false){
        ...
        echo '<div class="esg-filterbutton'.$sel.' esg-allfilter" data-filter="filterall" data-fid="-1" ><span>'.$this->filter_all_text['filter'].'</span></div>';
        ...
        if(!empty($this->filter)){
            foreach($this->filter as $filter_id => $filter){
                ...
                echo '<div class="esg-filterbutton'.$sel.'" data-fid="'.$filter_id.'" data-filter="filter-'.sanitize_key($filter['slug']).'"><span>'.$filter_text.'</span><span class="esg-filter-checked"><i class="eg-icon-ok-1"></i></span></div>';
            }
        }
        echo '</div>';
	...
	public function output_search_input(){
		echo '<div class="esg-filter-wrapper eg-search-wrapper'.$this->special_class.'"';
		if($this->spacing !== false) echo $this->spacing;
		echo '>';
		echo '<input name="eg-search-input-'.$this->grid_id.'" class="eg-search-input" type="text" value="" placeholder="'.$this->search_text.'">';
		echo '<span class="eg-search-submit"><i class="eg-icon-search"></i></span>';
		echo '<span class="eg-search-clean"><i class="eg-icon-cancel"></i></span>';
		echo '</div>';
	...
	public function check_for_search(){
		...
		if($this->filter_added === false && $this->search_found === true){
			echo '<!-- NEEDED FOR SEARCH -->';
			echo '<div class="esg-filter-wrapper" style="display:none;">';
			echo '<article class="esg-filters esg-'.$this->filter_type.' '.$this->special_class.'">';
