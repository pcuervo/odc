...
class WP_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		@set_time_limit(max(30, min(1800, (int) axiom_get_theme_option('admin_dummy_timeout'))));
		...
		echo '<br><b>'.__('Import authors ...', 'axiom').'</b><br>'; flush();
		...
		try {
			if ($this->start_from_id == 0) {
				echo '<br><b>'.__('Import categories ...', 'axiom').'</b><br>'; flush();
				$this->process_categories();
				echo '<br><b>'.__('Import tags ...', 'axiom').'</b><br>'; flush();
				$this->process_tags();
				echo '<br><b>'.__('Import terms ...', 'axiom').'</b><br>'; flush();
				$this->process_terms();
			}
			echo '<br><b>'.sprintf(__('Import posts %s ...', 'axiom'), ($this->start_from_id > 0 ? sprintf(__('(continue from ID = %s)', 'axiom'), $this->start_from_id) : '')).'</b><br>'; flush();
			$result = $this->process_posts();
		} catch (Exception $e) {
			$error = true;
			echo '<br><br><b>'.sprintf(__('Error while import: %s', 'axiom'), $e->getMessage()).'</b><br>'; flush();
		}
		...
		if ($result >= 100) {
			echo '<br><b>'.__('Backfill parents and attachments ...', 'axiom').'</b><br>'; flush();
	...
	function import_start( $file ) {
		if ( ! is_file($file) ) {
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'axiom' ) . '</strong><br />';
			echo __( 'The file does not exist, please try again.', 'axiom' ) . '</p>';
			$this->footer();
			die();
		}
	...
	function import_end() {
		...
		echo '<p>' . __( 'All done.', 'axiom' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'axiom' ) . '</a>' . '</p>';
		echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', 'axiom' ) . '</p>';
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'axiom' ) . '</strong><br />';
			printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'axiom' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'axiom' ), esc_html($import_data['version']) );
			echo '</strong></p></div>';
		}
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf( __( 'Failed to import author "%s". Their posts will be attributed to the current user.', 'axiom' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
...
<?php if ( ! empty( $this->authors ) ) : ?>
	<h3><?php _e( 'Assign Authors', 'axiom' ); ?></h3>
	<p><?php _e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', 'axiom' ); ?></p>
<?php if ( $this->allow_create_users() ) : ?>
	<p><?php printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'axiom' ), esc_html( get_option('default_role') ) ); ?></p>
<?php endif; ?>
	<ol id="authors">
...
<?php if ( $this->allow_fetch_attachments() ) : ?>
	<h3><?php _e( 'Import Attachments', 'axiom' ); ?></h3>
	<p>
		<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
		<label for="import-attachments"><?php _e( 'Download and import file attachments', 'axiom' ); ?></label>
	</p>
<?php endif; ?>

	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'axiom' ); ?>" /></p>
</form>
<?php
	}
	...
	function author_select( $n, $author ) {
		...
		if ( $create_users ) {
			if ( $this->version != '1.0' ) {
				_e( 'or create new user with login name:', 'axiom' );
				$value = '';
			} else {
				_e( 'as a new user:', 'axiom' );
		...
		if ( ! $create_users && $this->version == '1.0' )
			_e( 'assign posts to an existing user:', 'axiom' );
		else
			_e( 'or assign posts to an existing user:', 'axiom' );
		wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -', 'axiom' ) ) );
...
if ( $this->debug ) { printf( __( 'Author "%s" imported.', 'axiom' ).'<br>', esc_html($this->authors[$old_login]['author_display_name']) ); flush(); }
				} else {
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'axiom' ), esc_html($this->authors[$old_login]['author_display_name']) );
					if ( $this->debug )
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($cat['term_id']) ) {
					...
					if ( $this->debug ) { printf( __( 'Category "%s" imported.', 'axiom' ).'<br>', esc_html($cat['category_nicename']) ); flush(); }
				}
			} else {
				printf( __( 'Failed to import category "%s"', 'axiom' ), esc_html($cat['category_nicename']) );
				if ( $this->debug )
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($tag['term_id']) ) {
					...
					if ( $this->debug ) { printf( __( 'Tag "%s" imported.', 'axiom' ).'<br>', esc_html($tag['tag_name']) ); flush(); }
				}
			} else {
				printf( __( 'Failed to import post tag "%s"', 'axiom' ).'<br />', esc_html($tag['tag_name']) );
				if ( $this->debug ) {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($term['term_id']) ) {
					...
					if ( $this->debug ) { printf( __( 'Taxonomy "%s": term "%s" imported.', 'axiom' ).'<br>', esc_html($term['term_taxonomy']), esc_html($term['term_name']) ); flush(); }
				}
			} else {
				printf( __( 'Failed to import term %s: "%s"', 'axiom' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				if ( $this->debug )
					echo ': ' . ($id->get_error_message());
				echo '<br />';
				flush();
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				...
				printf( __( 'Failed to import post &#8220;%s&#8221;: Invalid post type %s', 'axiom' ),
				...
				do {	
					...
					if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
						...
						if ( is_wp_error( $post_id ) ) {
							printf( __( 'Failed to import post %s: &#8220;%s&#8221;', 'axiom' ),
						...
						if ( $this->debug ) { printf( __( '%s "%s" (ID=%s) imported.', 'axiom' ).'<br>', $post_type_object->labels->singular_name, esc_html($post['post_title']), $post_id ); flush(); }
					}
			...
			if (time() - $this->start_time >= $this->max_time || ($posts_counter + 1 >= $posts_all && time() - $this->start_time < $this->max_time/2)) break;
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			if ($this->debug) {
				printf(__( 'Menu item "%s" (id=%s) skipped due to missing menu slug', 'axiom' ), $item['post_title'], $item['post_id']);
				echo '<br />';
			}
			return;
		}
		...
		if ( ! $menu_id ) {
			if ($this->debug) {
				printf( __( 'Menu item "%s" (id=%s) skipped due to invalid menu slug: %s', 'axiom' ), $item['post_title'], $item['post_id'], esc_html( $menu_slug ) );
				echo '<br />';
			}
			return;
		} else {
		...
		if ($_menu_item_type == 'custom' && strpos($_menu_item_url, $this->demo_url) !== false) {
			$_menu_item_url = str_replace($this->demo_url, get_site_url() . (axiom_substr($this->demo_url, -1)=='/' ? '/' : ''), $_menu_item_url);
		}
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			if ( $this->debug ) { printf( __( 'Menu item "%s" (ID=%s) imported.', 'axiom' ).'<br>', $item['post_title'], $item['post_id'] ); flush(); }
		}
	}
	...
	function process_attachment( $post, $url ) {
		...
		if ( ! $this->fetch_attachments )
			return new WP_Error( 'attachment_processing_error',
				__( 'Fetching attachments is not enabled', 'axiom' ) );
		*/
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			return new WP_Error( 'attachment_processing_error', __('Invalid file type', 'axiom') );
...
if ($this->debug) printf( __('Attachment "%s" -', 'axiom'), $url);
		...
		if ( !file_exists($upload['file']) && $this->fetch_attachments ) {
			...
			if ( ! $headers ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Remote server did not respond', 'axiom') );
			}
	
			// make sure the fetch was successful
			if ( $headers['response'] != '200' ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'axiom'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
			}
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Remote file is incorrect size', 'axiom') );
			}
	
			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'axiom') );
			}
	
			$max_size = (int) $this->max_attachment_size();
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf(__('Remote file is too large, limit is %s', 'axiom'), size_format($max_size) ) );
			}
...
if ($this->debug) echo ' '.__('fetched', 'axiom').'<br>';
		} else
if ($this->debug) echo ' '.__('already uploaded', 'axiom').'<br>';
	...
	function replace_uploads($str) {
		if (is_array($str)) {
			...
			while (($pos = axiom_strpos($str, "/{$this->uploads_folder}/"))!==false) {
				$pos0 = $pos;
				while ($pos0 > 0) {
				...
				$str = ($pos0 > 0 ? axiom_substr($str, 0, $pos0) : '') . ($this->uploads_url) . axiom_substr($str, $pos+axiom_strlen($this->uploads_folder)+1);
			}
		}
		return $str;
	}
	...
	function header() {
		echo '<div class="wrap">';
		echo '<h2>' . __( 'Import WordPress', 'axiom' ) . '</h2>';
		...
		if ( isset( $updates[$basename] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'axiom' ), $update->update->new_version );
			echo '</strong></p></div>';
		}
	}
	...
	function greet() {
		...
		echo '<p>'.__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'axiom' ).'</p>';
		echo '<p>'.__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'axiom' ).'</p>';
