<?php
/**
 * AxiomThemes Framework: media (images, galleries, video and audio) manipulations
 *
 * @package	axiom
 * @since	axiom 1.0
 */
...
// Theme init
if (!function_exists('axiom_media_theme_setup')) {
	add_action( 'axiom_action_before_init_theme', 'axiom_media_theme_setup' );
	function axiom_media_theme_setup() {

		// Substitute audio, video and galleries in widget text
		add_filter( 'widget_text',						'axiom_widget_text_substitutes' );

		// AJAX: Get attachment url
		add_action('wp_ajax_get_attachment_url',		'axiom_callback_get_attachment_url');
		add_action('wp_ajax_nopriv_get_attachment_url',	'axiom_callback_get_attachment_url');
	}
}
...
if (!function_exists('axiom_get_post_image')) {
	function axiom_get_post_image($post_text, $get_src=true) {
		$src = '';
		$tags = array('<img>', '[trx_image]', '[image]');
		for ($i=0; $i<count($tags); $i++) {
			$tag = $tags[$i];
			if (($pos_start = axiom_strpos($post_text, axiom_substr($tag, 0, -1).' '))!==false) {
				$pos_end = axiom_strpos($post_text, axiom_substr($tag, -1), $pos_start);
				$tag_text = axiom_substr($post_text, $pos_start, $pos_end-$pos_start+1);
				if ($get_src) {
		...
		if ($src == '' && $get_src) $src = axiom_get_first_url($post_text);
		return $src;
	}
}

// Return gallery tag from photos array
if (!function_exists('axiom_build_gallery_tag')) {
	function axiom_build_gallery_tag($photos, $w, $h, $zoom=false, $link='') {
		...
		if (count($photos) > 0) {
			...
			$gallery_text = '
				<div id="'.esc_attr($id).'" class="sc_slider sc_slider_'.esc_attr($engine)
					...
					.'"'
					.(!empty($w) && axiom_strpos($w, '%')===false ? ' data-old-width="' . esc_attr($w) . '"' : '')
					.(!empty($h) && axiom_strpos($h, '%')===false ? ' data-old-height="' . esc_attr($h) . '"' : '')
			...
			foreach ($photos as $photo) {
				...
				if ($gallery_items_in_bg) {
					$photo_min = axiom_get_resized_image_url($photo, $w, $h);
					$gallery_text .= '<div' 
						. ' class="'.esc_attr($engine).'-slide"'
						. ' style="background-image:url(' . esc_url($photo_min) . ');'
						. (!empty($w) ? 'width:' . esc_attr($w) . (axiom_strpos($w, '%')!==false ? '' : 'px').';' : '')
						. (!empty($h) ? 'height:' . esc_attr($h) . (axiom_strpos($h, '%')!==false ? '' : 'px').';' : '')
						. '">' 
						. ($zoom ? '<a href="'.esc_url($photo).'"></a>' : ($link ? '<a href="'.esc_url($link).'"></a>' : '')) 
						. '</div>';
				} else {
					$photo_min = axiom_get_resized_image_tag($photo, $w, $h);
					$gallery_text .= '<div'
...
if (!function_exists('axiom_get_post_gallery')) {
	function axiom_get_post_gallery($text, $id=0, $max_slides=-1) {
		$rez = array();
		$ids = array();
		$ids_list = $orderby = '';
		if ($id > 0) {		// Get by post id
			...
			$tag = '[gallery]';
			$ids_list = axiom_get_tag_attrib($text, $tag, 'ids');
			$orderby  = axiom_get_tag_attrib($text, $tag, 'orderby');
		}
		if ($ids_list!='') {
...
if (!function_exists('axiom_substitute_gallery')) {
	function axiom_substitute_gallery($post_text, $post_id, $w, $h, $a='none', $zoom=false) {
		$tag = '[gallery]';
		$post_photos = false;
		while (($pos_start = axiom_strpos($post_text, axiom_substr($tag, 0, -1)))!==false) {
			$pos_end = axiom_strpos($post_text, axiom_substr($tag, -1), $pos_start);
			$tag_text = axiom_substr($post_text, $pos_start, $pos_end-$pos_start+1);
			if (($ids = axiom_get_tag_attrib($tag_text, $tag, 'ids'))!='') {
				...
				if ($post_photos===false)
					$post_photos = axiom_get_post_gallery('', $post_id);
				$photos = $post_photos;
			}
			
			$post_text = axiom_substr($post_text, 0, $pos_start) . axiom_build_gallery_tag($photos, $w, $h, $zoom) . axiom_substr($post_text, $pos_end + 1);
		}
		return $post_text;
	}
}
...
if (!function_exists('axiom_get_resized_image_tag')) {
	function axiom_get_resized_image_tag( $post, $w=null, $h=null, $c=null, $u=true, $find_thumb=false, $itemprop=false ) {
		static $mult = 0;
		if ($mult == 0) $mult = min(2, max(1, axiom_get_theme_option("retina_ready")));
		if (is_object($post))		$alt = axiom_get_post_title( $post->ID );
		else if ((int) $post > 0) 	$alt = axiom_get_post_title( $post );
...
if (!function_exists('axiom_get_resized_image_url')) {
	function axiom_get_resized_image_url( $post, $w=null, $h=null, $c=null, $u=true, $find_thumb=false ) {
		$url = '';
		if (is_object($post) || abs((int) $post) != 0) {
			...
			if ($url == '' && $find_thumb) {
				if (($data = get_post(is_object($post) ? $post->ID : $post))!==null) {
					$url = axiom_get_tag_attrib($data->post_content, '<img>', 'src');
				}
			}
		} else
			$url = trim(chop($post));
		if ($url != '') {
			if (axiom_strpos($url, '<img')!==false) {
				$url = axiom_get_tag_attrib($url, '<img>', 'src');
			}
			if ($c === null) $c = true;	//$c = get_option('thumbnail_crop')==1;
			if ( ! ($new_url = axiom_resize_image( $url, $w, $h, $c, true, $u)) ) $new_url = true ? $url : get_the_post_thumbnail($url, array($w, $h));
		} else 
			$new_url = '';
		return $new_url;
	}
...
if (!function_exists('axiom_resize_image')) {
	function axiom_resize_image( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false ) {
	
		// Validate inputs.
		if ( ! $url || ( ! $width && ! $height ) ) return false;
	
		// Caipt'n, ready to hook.
		if ( true === $upscale ) add_filter( 'image_resize_dimensions', 'axiom_resize_image_upscale', 10, 6 );
		...
		// Okay, leave the ship.
		if ( true === $upscale ) remove_filter( 'image_resize_dimensions', 'axiom_resize_image_upscale' );
...
if (!function_exists('axiom_resize_image_upscale')) {
	function axiom_resize_image_upscale( $default, $orig_w, $orig_h, $dest_w, $dest_h, $crop ) {
...
if (!function_exists('axiom_get_post_audio')) {
	function axiom_get_post_audio($post_text, $get_src=true) {
		$src = '';
		$tags = array('<audio>', '[trx_audio]', '[audio]');
		for ($i=0; $i<count($tags); $i++) {
			$tag = $tags[$i];
			$tag_end = axiom_substr($tag,0,1) . '/' . axiom_substr($tag,1);
			if (($pos_start = axiom_strpos($post_text, axiom_substr($tag, 0, -1).' '))!==false) {
				$pos_end = axiom_strpos($post_text, axiom_substr($tag, -1), $pos_start);
				...
				if ($get_src) {
					if (($src = axiom_get_tag_attrib($tag_text, $tag, 'src'))=='') {
						if (($src = axiom_get_tag_attrib($tag_text, $tag, 'url'))=='' && $i==1) {
...
if (!function_exists('axiom_substitute_audio')) {
	function axiom_substitute_audio($post_text, $in_frame=true) {
		...
		while (($pos_start = axiom_strpos($post_text, axiom_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			if ($src != '') {
				...
				if (!$tag_a) $tag_a = axiom_get_tag_attrib($tag_text, $tag, 'data-align');
				$tag_s = axiom_get_tag_attrib($tag_text, $tag, 'style')
						. ($tag_w!='' ? 'width:' . esc_attr($tag_w) . (axiom_substr($tag_w, -1)!='%' ? 'px' : '') . ';' : '')
				...
				$tag_text = '<div'.($id ? ' id="'.esc_attr($id).'"' : '') . ' class="sc_audio_container' . (!$in_frame && $tag_a ? ' align'.esc_attr($tag_a) : '') . '">'
					. (axiom_strpos($src, 'soundcloud.com') !== false
				...
				$post_text = axiom_substr($post_text, 0, (axiom_substr($post_text, $pos_start-3, 3)=='<p>' ? $pos_start-3 : $pos_start))
					. ($tag_text)
					. axiom_substr($post_text, (axiom_substr($post_text, $pos_end, 4)=='</p>' ? $pos_end+4 : $pos_end));
...
if (!function_exists('axiom_get_audio_frame')) {
	function axiom_get_audio_frame($audio, $image_thumb='', $style='') {
		$tag    = '<audio>';
		$tag_w  = axiom_get_tag_attrib($audio, $tag, 'width');
		$tag_h  = axiom_get_tag_attrib($audio, $tag, 'height');
		$class  = axiom_get_tag_attrib($audio, $tag, 'class');
		$author = axiom_get_tag_attrib($audio, $tag, 'data-author');
		$title  = axiom_get_tag_attrib($audio, $tag, 'data-title');
		$anim   = axiom_get_tag_attrib($audio, $tag, 'data-animation');
		$align  = axiom_get_tag_attrib($audio, $tag, 'data-align');
		$image  = axiom_get_tag_attrib($audio, $tag, 'data-image');
		...
		if( !empty($title) || !empty($author) ){
			$header = '<div class="sc_audio_header">'
						...
						. ($author != '' ? '<div class="sc_audio_author"><span class="sc_audio_author_by">'.__('by', 'axiom').' </span><span class="sc_audio_author_name">'.($author).'</span></div>' : '')
					.'</div>';
		}
...
if (!function_exists('axiom_get_post_video')) {
	function axiom_get_post_video($post_text, $get_src=true) {
		$src = '';
		...
		if ($src == '' && $get_src) $src = axiom_get_first_url($post_text);
		//if (!axiom_is_youtube_url($src) && !axiom_is_vimeo_url($src)) $src = '';
		return $src;
	}
}

// Substitute video tags and shortcodes
if (!function_exists('axiom_substitute_video')) {
	function axiom_substitute_video($post_text, $w, $h, $in_frame=true) {
		...
		while (($pos_start = axiom_strpos($post_text, axiom_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			$pos_end2 = axiom_strpos($post_text, $tag_end, $pos_end);
			$tag_text = axiom_substr($post_text, $pos_start, ($pos_end2!==false ? $pos_end2+axiom_strlen($tag_end)-1 : $pos_end)-$pos_start+1);
			if (axiom_get_tag_attrib($tag_text, $tag, 'data-frame')=='no') continue;
			if (($src = axiom_get_tag_attrib($tag_text, $tag, 'src'))=='')
				$src = axiom_get_tag_attrib($tag_text, $tag, 'url');
			if ($src != '') {
				$auto = axiom_get_tag_attrib($tag_text, $tag, 'autoplay');
				$src = axiom_get_video_player_url($src, $auto!=''); // && is_single());
				$id = axiom_get_tag_attrib($tag_text, $tag, 'id');
				$tag_w = axiom_get_tag_attrib($tag_text, $tag, 'width');
				$tag_h = axiom_get_tag_attrib($tag_text, $tag, 'height');
				$tag_a = axiom_get_tag_attrib($tag_text, $tag, 'align');
...
if (!function_exists('axiom_get_video_frame')) {
	function axiom_get_video_frame($video, $image='', $style='', $style2='') {
		$tag   = axiom_strpos($video, '<iframe')!==false ? '<iframe>' : '<video>';
		$tag_w = axiom_get_tag_attrib($video, $tag, 'width');
		$tag_h = axiom_get_tag_attrib($video, $tag, 'height');
		$tag_anim = axiom_get_tag_attrib($video, $tag, 'data-animation');
		$tag_align = axiom_get_tag_attrib($video, $tag, 'data-align');
		$video = '<div class="sc_video_player'.($style2 ? ' sc_video_bordered' : '').($tag_align ? ' align'.esc_attr($tag_align) : '').'"' 
				...
				. '<div class="sc_video_frame' . ($image ? ' sc_video_play_button hover_icon hover_icon_play' : '') . '"' 
					...
					. ($style ? ' style="' . esc_attr($style) . '"' : '')
					. '>'
					. ($image ? (axiom_strpos($image, '<img')!==false ? $image : '<img alt="" src="'.esc_url($image).'">') : $video)
				. '</div>'
			. '</div>';
		return $video;
	}
...
if (!function_exists('axiom_get_video_player_url')) {
	function axiom_get_video_player_url($url, $autoplay=false) {
		$url = str_replace(
			array(
				'http://youtu.be/',
				'http://www.youtu.be/',
			...
			trim(chop($url)));
		if ($autoplay && $url!='') {
			if (axiom_strpos($url, 'autoplay')===false) {
				$url .= (axiom_strpos($url, '?')===false ? '?' : '&') . 'autoplay=1';
			}
		}
		return $url;
	}
...
if (!function_exists('axiom_get_video_cover_image')) {
	function axiom_get_video_cover_image($url) {
		$image = '';
		if (axiom_is_youtube_url($url)) {
			$parts = parse_url($url);
...
if (!function_exists('axiom_is_youtube_url')) {
	function axiom_is_youtube_url($url) {
		return axiom_strpos($url, 'youtu.be')!==false || axiom_strpos($url, 'youtube.com')!==false;
	}
}

if (!function_exists('axiom_is_vimeo_url')) {
	function axiom_is_vimeo_url($url) {
		return axiom_strpos($url, 'vimeo.com')!==false;
	}
}
...
if (!function_exists('axiom_substitute_all')) {
	function axiom_substitute_all($text, $w=275, $h=200) {
		if (axiom_get_custom_option('substitute_gallery')=='yes') {
			$text = axiom_substitute_gallery($text, 0, $w, $h);
		}
		$text = do_shortcode(apply_filters('axiom_filter_sc_clear_around', $text));
		if (axiom_get_custom_option('substitute_video')=='yes') {
			$text = axiom_substitute_video($text, $w, $h);
		}
		if (axiom_get_custom_option('substitute_audio')=='yes') {
			$text = axiom_substitute_audio($text);
		}
		return $text;
	}
}
...
// Substitute audio, video and galleries in widget text
if ( !function_exists( 'axiom_widget_text_substitutes' ) ) {
	function axiom_widget_text_substitutes( $text ){
		return axiom_substitute_all($text);
	}
}

// Return url from tag a, inserted in post
if (!function_exists('axiom_get_post_link')) {
	function axiom_get_post_link($post_text) {
		$src = '';
		$target = '';
		$tag = '<a>';
		$tag_end = '</a>';
		if (($pos_start = axiom_strpos($post_text, axiom_substr($tag, 0, -1).' '))!==false) {
			...
			$pos_end2 = axiom_strpos($post_text, $tag_end, $pos_end);
			$tag_text = axiom_substr($post_text, $pos_start, ($pos_end2!==false ? $pos_end2+7 : $pos_end)-$pos_start+1);
			$src = axiom_get_tag_attrib($tag_text, $tag, 'href');
			$target = axiom_get_tag_attrib($tag_text, $tag, 'target');
		}
		if ($src == '') $src = axiom_get_first_url($post_text);
		return array('url'=>$src, 'target'=>$target);
	}
}

// Return first url from post content
if (!function_exists('axiom_get_first_url')) {
