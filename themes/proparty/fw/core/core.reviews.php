<?php
/**
 * AxiomThemes Framework: Reviews support
 *
 * @package	axiom
 * @since	axiom 1.0
 */
...
if (!function_exists('axiom_reviews_theme_setup')) {
	add_action( 'axiom_action_before_init_theme', 'axiom_reviews_theme_setup' );
	function axiom_reviews_theme_setup() {
		// Get reviews criterias list from categories list (ids)
		add_action('wp_ajax_check_reviews_criterias',			'axiom_reviews_check_criterias');
		add_action('wp_ajax_nopriv_check_reviews_criterias',	'axiom_reviews_check_criterias');

		// Accept user's votes
		add_action('wp_ajax_reviews_users_accept',				'axiom_reviews_accept_user_marks');
		add_action('wp_ajax_nopriv_reviews_users_accept',		'axiom_reviews_accept_user_marks');
	}
}

// Get reviews criterias list from categories list (ids)
if ( !function_exists( 'axiom_reviews_check_criterias' ) ) {
	//add_action('wp_ajax_check_reviews_criterias', 'axiom_reviews_check_criterias');
	//add_action('wp_ajax_nopriv_check_reviews_criterias', 'axiom_reviews_check_criterias');
...
// Accept user's votes
if ( !function_exists( 'axiom_reviews_accept_user_marks' ) ) {
	//add_action('wp_ajax_reviews_users_accept', 'axiom_reviews_accept_user_marks');
	//add_action('wp_ajax_nopriv_reviews_users_accept', 'axiom_reviews_accept_user_marks');
	function axiom_reviews_accept_user_marks() {
		global $_REQUEST;
		...
		if ($post_id > 0) {
			...
			$avg = axiom_reviews_get_average_rating($marks);
			update_post_meta($post_id, 'reviews_marks2', axiom_reviews_marks_to_save($marks));
			update_post_meta($post_id, 'reviews_avg2', axiom_reviews_marks_to_save($avg));
			update_post_meta($post_id, 'reviews_users', $users);
		} else {
			$response['error'] = __('Bad post ID', 'axiom');
		}
...
if ( !function_exists( 'axiom_reviews_get_average_rating' ) ) {
	function axiom_reviews_get_average_rating($marks) {
		$r = explode(',', $marks);
		$rez = 0;
		$cnt = 0;
		foreach ($r as $v) {
...
if ( !function_exists( 'axiom_reviews_get_word_value' ) ) {
	function axiom_reviews_get_word_value($r, $words = '') {
		$max_level = max(5, (int) axiom_get_custom_option('reviews_max_level'));
		if (trim($words) == '') $words = axiom_get_theme_option('reviews_criterias_levels');
		$words = explode(',', $words);
		$k = $max_level / count($words);
		$r = max(0, min(count($words)-1, floor($r/$k)));
		return isset($words[$r]) ? trim($words[$r]) : __('no rated', 'axiom');
	}
}

// Return Reviews markup html-block
if ( !function_exists( 'axiom_reviews_get_markup' ) ) {
	function axiom_reviews_get_markup($field, $value, $editable=false, $clear=false, $snippets=false) {
		...
		if (is_array($criterias) && count($criterias) > 0) {
			...
			foreach ($criterias as $num=>$sb) { 
				...
				if ($clear || !isset($marks[$i]) || $marks[$i]=='' || axiom_is_inherit_option($marks[$i])) $marks[$i] = 0;
				...
				$output .= '<div class="reviews_item reviews_max_level_'.esc_attr($max_level).'" data-max-level="'.esc_attr($max_level).'" data-step="'.esc_attr($step).'">'
					...
					. trim(axiom_reviews_get_summary_stars($marks[$i], $editable))
					.'</div>';
				$i++;
			}
		}
...
if ( !function_exists( 'axiom_reviews_get_summary_stars' ) ) {
	function axiom_reviews_get_summary_stars($avg, $editable=false, $snippets=false, $stars_show=0) {
		$max_level = max(5, (int) axiom_get_custom_option('reviews_max_level'));
		$stars_count = min(10, $stars_show ? $stars_show : $max_level);
		$style = axiom_get_theme_option('reviews_style');
		...
		if (!$editable && $style=='text') {
			$output .= sprintf($max_level<100 ? __('%s / %s', 'axiom') : __('%s', 'axiom'), number_format($avg, 1) . ($max_level < 100 ? '' : '%'), $max_level . ($max_level < 100 ? '' : '%'));
		} else {
			$stars = axiom_strrepeat('<span class="reviews_star"></span>', $stars_count);
...
if ( !function_exists( 'axiom_reviews_marks_prepare' ) ) {
	function axiom_reviews_marks_prepare($marks, $cnt) {
		$m = explode(',', $marks);
		for ($i=0; $i < $cnt; $i++) {
			if (!isset($m[$i]))
				$m[$i] = 0;
...
if ( !function_exists( 'axiom_reviews_marks_to_save' ) ) {
	function axiom_reviews_marks_to_save($marks) {
		$max_level = max(5, (int) axiom_get_custom_option('reviews_max_level'));
...
if ( !function_exists( 'axiom_reviews_marks_to_display' ) ) {
	function axiom_reviews_marks_to_display($marks) {
		$max_level = max(5, (int) axiom_get_custom_option('reviews_max_level'));
		if ($max_level == 100) return $marks;
		$m = explode(',', $marks);
		$kol = count($m);
