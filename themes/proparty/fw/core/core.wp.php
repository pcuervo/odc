<?php
/**
 * AxiomThemes Framework: WordPress utilities
 *
 * @package	axiom
 * @since	axiom 1.0
 */
...
// Theme init
if (!function_exists('axiom_wp_theme_setup')) {
	add_action( 'axiom_action_before_init_theme', 'axiom_wp_theme_setup' );
	function axiom_wp_theme_setup() {

		// Pre query: Add posts per page value into wp query
		add_action( 'pre_get_posts', 						'axiom_query_add_posts_per_page' );

		// Pre query: Add filter to main query
		add_filter('posts_where', 							'axiom_query_posts_where', 10, 2);

		// Filter categories list - exclude unwanted cats from widget output
		add_action('widget_categories_args',				'axiom_query_exclude_categories');
		add_action('widget_categories_dropdown_args',		'axiom_query_exclude_categories');
		add_action('widget_posts_args',						'axiom_query_exclude_posts' );

		// AJAX: Get next page on blog streampage
		add_action('wp_ajax_view_more_posts', 				'axiom_callback_view_more_posts');
		add_action('wp_ajax_nopriv_view_more_posts',		'axiom_callback_view_more_posts');

		// AJAX: Incremental search
		add_action('wp_ajax_ajax_search',					'axiom_callback_ajax_search');
		add_action('wp_ajax_nopriv_ajax_search',			'axiom_callback_ajax_search');

		// AJAX: Change month in the calendar widget
		add_action('wp_ajax_calendar_change_month',			'axiom_callback_calendar_change_month');
		add_action('wp_ajax_nopriv_calendar_change_month',	'axiom_callback_calendar_change_month');

		// AJAX: Set post likes/views count
		add_action('wp_ajax_post_counter', 					'axiom_callback_post_counter');
		add_action('wp_ajax_nopriv_post_counter', 			'axiom_callback_post_counter');

		// AJAX: New user registration
		add_action('wp_ajax_registration_user',				'axiom_callback_registration_user');
		add_action('wp_ajax_nopriv_registration_user',		'axiom_callback_registration_user');

		// User login by email
		add_filter('authenticate',							'axiom_allow_email_login', 20, 3);

		// Add categories (taxonomies) filter for custom posts types
		add_action( 'restrict_manage_posts', 				'axiom_show_taxonomies_filters' );

		// Clear taxonomies cache when save or delete post or category
		add_action( 'save_post', 							'axiom_clear_cache_all', 10, 2 );
		add_action( 'delete_post', 							'axiom_clear_cache_all', 10, 2 );
		add_action( 'edit_category', 						'axiom_clear_cache_categories',   10, 1 );
		add_action( 'create_category', 						'axiom_clear_cache_categories', 10, 1 );
		add_action( 'delete_category', 						'axiom_clear_cache_categories', 10, 1 );

		// Clear WP categories cache 
		add_action('wp_ajax_clear_cache',					'axiom_callback_clear_cache');
		add_action('wp_ajax_nopriv_clear_cache', 			'axiom_callback_clear_cache');
	}
}
...
if (!function_exists('axiom_get_blog_type')) {
	function axiom_get_blog_type($query=null) {
		global $wp_query;
		static $page='';
		if (!empty($page)) return $page;
		if ( $query===null ) $query = $wp_query;
		$page = apply_filters('axiom_filter_get_blog_type', $page, $query);
		return $page;
	}
}

// Return blog title
if (!function_exists('axiom_get_blog_title')) {
	function axiom_get_blog_title() {
		global $wp_query;
		static $title='';
		if (!empty($title)) return $title;
		$page  = axiom_get_blog_type();
		$title = apply_filters('axiom_filter_get_blog_title', $title, $page);
		return $title;
	}
}

// Show breadcrumbs path
if (!function_exists('axiom_show_breadcrumbs')) {
	function axiom_show_breadcrumbs($args=array()) {
		...
		$args = array_merge( array(
			'home' => __('Home', 'axiom'),		// Home page title (if empty - not showed)
			...
			'max_levels' => axiom_get_theme_option('breadcrumbs_max_level'),		// Max categories in the path (0 - unlimited)
			...
			while ($pageParentID > 0) {
				...
				if ($level > $args['max_levels'])
					...
					$rez_parent = '<a class="breadcrumbs_item cat_post" href="' . get_permalink($pageParent->ID) . '">' 
									. trim(axiom_strshort($pageParent->post_title, $args['truncate_title'], $args['truncate_add']))
									. '</a>' 
							...
							if ($level > $args['max_levels'])
								...
								$rez_all .= (!empty($rez_all) ? $args['delimiter'] : '') 
										. '<a class="breadcrumbs_item all" href="' . get_permalink($tpl_id) . '">' 
											. sprintf(__('All %s', 'axiom'), axiom_get_post_format_name($post_format, false))
										. '</a>';
						}
					}
					$cats = axiom_get_terms_by_post_id( array(
						'post_id'  => $post_id, 
						'taxonomy' => $taxonomy,
						'exclude'  => $ex_cats
						)
					...
					if ($cat) {
						...
						if ($level > $args['max_levels'])
							...
							$rez_parent = '<a class="breadcrumbs_item cat_post" href="'.esc_url($cat->link).'">' 
											. trim(axiom_strshort($cat->name, $args['truncate_title'], $args['truncate_add']))
											. '</a>' 
											. (!empty($rez_parent) ? $args['delimiter'] : '') 
											. ($rez_parent);
					}
...
if (!function_exists('axiom_show_pagination')) {
	function axiom_show_pagination($args=array()) {
		$args = array_merge(array(
			...
			'show_pages' => 5,
			'pages_in_group' => 10,
			'pages_text' => '', 		//__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'axiom'),
			'current_text' => "%PAGE_NUMBER%",
			'page_text' => "%PAGE_NUMBER%",
			'first_text'=> __('&laquo; First', 'axiom'),
			'last_text' => __("Last &raquo;", 'axiom'),
			'prev_text' => __("&laquo; Prev", 'axiom'),
			'next_text' => __("Next &raquo;", 'axiom'),
			'dot_text' => "&hellip;",
			'before' => '',
			'after' => '',
			),  is_array($args) ? $args 
		...
		if (!is_single()) {
			axiom_show_pagination_blog($args);
		} else {
			axiom_show_pagination_single($args);
		}
	}
}

// Single page navigation
if (!function_exists('axiom_show_pagination_single')) {
	function axiom_show_pagination_single( $args ) {
		global $wp_query, $post;
		...
		if ( is_single() ) {
			?>
			<h3 class="screen-reader-text"><?php _e( 'Post navigation', 'axiom' ); ?></h3>
			<?php
...
// Blog pagination
if (!function_exists('axiom_show_pagination_blog')) {
...
if ( !function_exists( 'axiom_callback_view_more_posts' ) ) {
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			while ( have_posts() ) { the_post(); 
				...
				$post_args = array(
					...
					// Get post data
					'content' => axiom_get_template_property($vars['blog_style'], 'need_content'),
					'terms_list' => !axiom_sc_param_is_off($vars['filters']) || axiom_get_template_property($vars['blog_style'], 'need_terms'),
					'strip_teaser' => false,
...
// Incremental search
if ( !function_exists( 'axiom_callback_ajax_search' ) ) {
	function axiom_callback_ajax_search() {
		...
		if (!empty($s)) {
			...
			$show_date 		= axiom_get_theme_option('ajax_search_posts_date')=='yes' ? 1 : 0;
			$show_image 	= axiom_get_theme_option('ajax_search_posts_image')=='yes' ? 1 : 0;
			$show_author 	= axiom_get_theme_option('ajax_search_posts_author')=='yes' ? 1 : 0;
			$show_counters	= axiom_get_theme_option('ajax_search_posts_counters')=='yes' ? axiom_get_theme_option('blog_counters') : '';
			$args = array(
				'post_status' => 'publish',
				'orderby' => 'date',
				'order' => 'desc', 
				'posts_per_page' => max(1, min(10, axiom_get_theme_option('ajax_search_posts_count'))),
				's' => esc_html($s),
				);
			// Filter post types
			if (!empty($show_types)) $args['post_type'] = explode(',', $show_types);
			// Exclude categories
			if (axiom_strpos($show_types, 'post') !== false) {
			...
			if (empty($output)) {
				$output .= '<article class="post_item">' . __('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'axiom') . '</article>';
			} else {
				$output .= '<article class="post_item"><a href="#" class="post_more">' . __('More results &hellip;', 'axiom') . '</a></article>';
			}
			$response['data'] = $output;
		} else {
			$response['error'] = __('The query string is empty!', 'axiom');
		}
...
if (!function_exists('axiom_get_calendar')) {
	function axiom_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		$prev = apply_filters('axiom_filter_calendar_get_prev_month', array(), array(
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'axiom'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		// Get days with posts
		$posts = apply_filters('axiom_filter_calendar_get_curr_month_posts', array(), array(
			'posts_types' => $posts_types,
			'year' => $thisyear,
			'month' => $thismonth,
			'last_day' => $last_day
		...
		if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
			...
			$link = apply_filters('axiom_filter_calendar_get_month_link', '', array(
				'posts_types' => $posts_types,
				'year' => $thisyear,
				'month' => $thismonth,
				'last_day' => $last_day
		...
		$next = apply_filters('axiom_filter_calendar_get_next_month', array(), array(
			'posts_types' => $posts_types,
			'year' => $thisyear,
			'month' => $thismonth,
			'last_day' => $last_day
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'axiom'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">'
...
if ( !function_exists( 'axiom_callback_calendar_change_month' ) ) {
...
if (!function_exists('axiom_output_single_comment')) {
	function axiom_output_single_comment( $comment, $args, $depth ) {
		$GLOBALS['comment'] = $comment;
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php _e( 'Trackback:', 'axiom' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'axiom' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php _e( 'Pingback:', 'axiom' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'axiom' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					<div class="comment_author_avatar"><?php echo get_avatar( $comment, 75*min(2, max(1, axiom_get_theme_option("retina_ready")))); ?></div>
					<div class="comment_content">
						<div class="comment_info">
							<span class="comment_author"><?php echo ($author_id ? '<a href="'.esc_url($author_link).'">' : '') . comment_author() . ($author_id ? '</a>' : ''); ?></span>
							<span class="comment_date"><span class="comment_date_label"><?php _e('Posted', 'axiom'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
							<span class="comment_time"><?php echo get_comment_date(get_option('time_format')); ?></span>
						</div>
						<div class="comment_text_wrap">
							<?php if ( $comment->comment_approved == 0 ) { ?>
							<div class="comment_not_approved"><?php _e( 'Your comment is awaiting moderation.', 'axiom' ); ?></div>
							<?php } ?>
...
if (!function_exists('axiom_get_template_page_id')) {
	function axiom_get_template_page_id($name) {
		$posts = axiom_get_posts_by_meta_value('_wp_page_template', $name . '.php');
		$id = 0;
...
if (!function_exists('axiom_detect_template_page_id')) {
	function axiom_detect_template_page_id() {
		...
		if (empty($template_id)) $template_id = apply_filters('axiom_filter_detect_template_page_id', 0, $key);
		return $template_id;
	}
}
	
// Return ID of the stream page for specified mode
if (!function_exists('axiom_get_stream_page_id')) {
	function axiom_get_stream_page_id($mode) {
		return apply_filters('axiom_filter_get_stream_page_id', '', $mode);
	}
}
	
// Return link to the stream page for specified mode
if (!function_exists('axiom_get_stream_page_link')) {
	function axiom_get_stream_page_link($mode) {
		return apply_filters('axiom_filter_get_stream_page_link', '', $mode);
	}
}
	
// Return title of the stream page for specified mode
if (!function_exists('axiom_get_stream_page_title')) {
	function axiom_get_stream_page_title($mode) {
		return apply_filters('axiom_filter_get_stream_page_title', '', $mode);
	}
}

// Return taxonomy name (slug) if current page is this taxonomy page
if ( !function_exists( 'axiom_is_taxonomy' ) ) {
	function axiom_is_taxonomy($query=null) {
		static $tax = '';
		if (!empty($tax)) return $tax;
		$tax = apply_filters('axiom_filter_is_taxonomy',	$tax);
		if (!empty($tax)) return $tax;
...
if ( !function_exists( 'axiom_get_current_term' ) ) {
	function axiom_get_current_term($tax='') {
		static $term = null;
...
if (!function_exists('axiom_get_posts_by_meta_value')) {
	function axiom_get_posts_by_meta_value($meta_key, $meta_value, $count=1) {
		global $post;
		$args = array(
...
if (!function_exists('axiom_get_terms_by_post_id')) {
	function axiom_get_terms_by_post_id($args=array()) {
		$args = array_merge(array(
			'post_id' 	=> 0,
			'taxonomy' 	=> array(),
			'parent_id'	=> 0,
		...
		$closest_method = axiom_get_theme_option('close_category');
		// make terms list
		$terms_list = array();
				...
				if (!empty($terms)) {
					...
					foreach ( $terms as $term ) {
						...
						if ($taxonomy_obj->hierarchical && empty($terms_list[$taxonomy_slug]->closest_parent)) {
							$terms_list[$taxonomy_slug]->closest_parent = $closest_method == 'parental' ? axiom_get_parent_taxonomy($term->term_id, $args['parent_id'], $taxonomy_slug) : $term;
...
if (!function_exists('axiom_get_categories_by_post_id')) {
	function axiom_get_categories_by_post_id($args = array()) {
		if (!is_array($args)) $args = array('post_id' => $args);
		if (!isset($args['taxonomy'])) $args['taxonomy'] = 'category';
		return axiom_get_terms_by_post_id($args);
	}
}

// Return tags objects by post id
if (!function_exists('axiom_get_tags_by_post_id')) {
	function axiom_get_tags_by_post_id($args = array()) {
		if (!is_array($args)) $args = array('post_id' => $args);
...
if (!function_exists('axiom_get_terms_by_taxonomy')) {
	function axiom_get_terms_by_taxonomy($tax_types = 'post_format', $args=array()) {
...
if (!function_exists('axiom_get_parent_taxonomy')) {
	function axiom_get_parent_taxonomy($id, $parent_id=0, $taxonomy='category') {
		$val = null;
		do {
...
if (!function_exists('axiom_get_parent_category')) {
	function axiom_get_parent_category($id, $parent_id=0) {
		return axiom_get_parent_taxonomy($id, $parent_id, 'category');
	}
}

// Return id nearest (or highest) parent taxonomy with specified property in array values
if (!function_exists('axiom_get_parent_taxonomy_by_property')) {
...
if (!function_exists('axiom_get_parent_category_by_property')) {
...
if ( !function_exists( 'axiom_show_taxonomies_filters' ) ) {
	function axiom_show_taxonomies_filters() {
		if (axiom_get_theme_option('admin_add_filters')!='yes') return;
		...
		if ($page == 'post')
			...
			return;
		echo trim(axiom_get_terms_filters($taxes));
	}
}

// Return string with <select> tags for each taxonomy
if (!function_exists('axiom_get_terms_filters')) {
...
if (!function_exists('axiom_get_terms_hierarchical_list')) {
	function axiom_get_terms_hierarchical_list($terms, $opt=array()) {
		...
		if (count($terms) > 0) {
			foreach ($terms as $term) {
				...
				$rez = array_merge($rez, axiom_get_terms_hierarchical_list($terms, array(
...
if ( !function_exists( 'axiom_clear_cache_all' ) ) {
	function axiom_clear_cache_all($post_id=0, $post_obj=null) {
		delete_transient("axiom_terms_filter_category");
		delete_transient("axiom_terms_filter_post_tag");
		delete_transient("axiom_terms_filter_post_format");
	}
}

// Clear categories cache (when create, edit or delete category)
if ( !function_exists( 'axiom_clear_cache_categories' ) ) {
	function axiom_clear_cache_categories($term_id=0) {
		delete_transient("axiom_terms_filter_category");
	}
}
...
if (!function_exists('axiom_get_post_data_options')) {
	function axiom_get_post_data_options($opt) {
		$opt = array_merge(array(
			...
			'counters' => axiom_get_custom_option("blog_counters"),	// What counters use: views or comments
			...
			'sidebar' => !axiom_sc_param_is_off(axiom_get_custom_option('show_sidebar_main')),	// Sidebar is visible on current page
			'more_tag' => null,			// Text for the "Read more" link
			'strip_teaser' => axiom_get_custom_option('show_text_before_readmore')!='yes',	// Strip text before <!--more--> tag in the content or show full content
			'substitute_gallery' => axiom_get_custom_option('substitute_gallery')=='yes',	// Substitute standard WP gallery on theme slider
			'substitute_video'   => axiom_get_custom_option('substitute_video')=='yes',		// Substitute tag <video> on the Youtube or Vimeo player
			'substitute_audio'   => axiom_get_custom_option('substitute_audio')=='yes',		// Substitute tag <audio> on the Sound Cloud's player
...
if (!function_exists('axiom_show_post_layout')) {
...
if (!function_exists('axiom_get_post_data')) {
	function axiom_get_post_data(&$opt, $post_obj=null) {
		...
		$post_icon = axiom_get_custom_option('icon', axiom_get_post_format_icon($post_format), $post_id, $post_type);	//!!!!! Get option from specified post
		$post_flags = array(
			'sticky' => is_sticky()
		);
		...
		if ($opt['content']) {
			if ($opt['substitute_gallery'])	$post_content = axiom_substitute_gallery($post_content, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop'], 'none', true);
			$post_content = apply_filters('the_content', $post_content);
			if ($post_id != get_the_ID()) {		// Fix bug in the WPML
				$post = $cur_post;
				setup_postdata($post);
			}
			if ($opt['substitute_video']) 	$post_content = axiom_substitute_video($post_content, $thumb_sizes['w'], $thumb_sizes['h_crop']);
			if ($opt['substitute_audio'])	$post_content = axiom_substitute_audio($post_content);
		}
		...
		if ($opt['substitute_gallery']) $post_excerpt = axiom_substitute_gallery($post_excerpt, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop']);
		...
		if ($opt['substitute_video']) $post_excerpt = axiom_substitute_video($post_excerpt, $thumb_sizes['w'], $thumb_sizes['h_crop']);
		if ($opt['substitute_audio']) $post_excerpt = axiom_substitute_audio($post_excerpt);
		...
		if (empty($opt['location']) || $opt['location'] == 'default')
			$opt['location'] = axiom_get_custom_option('dedicated_location', '', $post_id, $post_type);	//!!!!! Get option from specified post
		...
		else if (axiom_substr($opt['layout'], 0, 10)=='attachment')
		...
		if ($post_format == 'gallery') {
			...
			if ($post_video=='') {
				...
				if (axiom_substr($post_video, 0, 1)=='[') {
					...
					if (empty($src)) $src = axiom_get_tag_attrib($post_video, '[trx_video]', 'url');
					if (empty($src)) $src = axiom_get_post_video($post_video, true);
			...
			if ($post_video!='' && $opt['substitute_video']) {
				$post_video = axiom_substitute_video($post_video, $thumb_sizes['w'], round($thumb_sizes['w']/16*9), false);	//$thumb_sizes['h_crop']);
			}
			...
			if ($post_audio=='') {
				...
				if (axiom_substr($post_audio, 0, 1)=='[') {
					...
					if (empty($src)) $src = axiom_get_tag_attrib($post_audio, '[trx_audio]', 'url');
					if (empty($src)) $src = axiom_get_post_audio($post_audio, true);
		...
		$post_terms = array();
		if ($opt['terms_list']) {
			$post_terms = axiom_get_terms_by_post_id( array(
				'post_id' 	=> $post_id,
				'parent_id'	=> $post_type=='post' ? $opt['parent_tax_id'] : 0,
				'exclude'	=> $post_type=='post' ? explode(',', axiom_get_theme_option('exclude_cats')) : array()
				)
			);
		}
...
if (!function_exists('axiom_get_post_title')) {
	function axiom_get_post_title($id = 0, $maxlength = 0, $add='...') {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$title = get_the_title($id);
		if ($maxlength > 0) $title = axiom_strshort($title, $maxlength, $add);
		return $title;
	}
}

// Return post excerpt
if (!function_exists('axiom_get_post_excerpt')) {
	function axiom_get_post_excerpt($maxlength = 0, $add='...') {
		...
		if (!empty($descr) && axiom_strpos(',.:;-', axiom_substr($descr, -1))!==false) $descr = axiom_substr($descr, 0, -1);
		if ($maxlength > 0) $descr = axiom_strshort($descr, $maxlength, $add);
		return $descr;
	}
}

//Return Post Views Count
if (!function_exists('axiom_get_post_views')) {
	function axiom_get_post_views($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
...
if (!function_exists('axiom_set_post_views')) {
	function axiom_set_post_views($id=0, $counter=-1) {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'post_views_count';
		$count = get_post_meta($id, $count_key, true);
...
if (!function_exists('axiom_get_post_likes')) {
	function axiom_get_post_likes($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
...
if (!function_exists('axiom_set_post_likes')) {
	function axiom_set_post_likes($id=0, $count=0) {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'post_likes_count';
		update_post_meta($id, $count_key, $count);
...
// add_action('wp_ajax_post_counter', 			'axiom_callback_post_counter');
// add_action('wp_ajax_nopriv_post_counter',	'axiom_callback_post_counter');
if ( !function_exists( 'axiom_callback_post_counter' ) ) {
...
// Pre query: Add filter to main query
if ( !function_exists( 'axiom_query_posts_where' ) ) {
	// add_filter('posts_where', 'axiom_query_posts_where', 10, 2);
...
if ( !function_exists( 'axiom_query_add_posts_per_page' ) ) {
	// add_action( 'pre_get_posts', 'axiom_query_add_posts_per_page' );
...
// add_action( 'widget_categories_args', 			'axiom_query_exclude_categories' );
// add_action( 'widget_categories_dropdown_args',	'axiom_query_exclude_categories' );
if ( !function_exists( 'axiom_query_exclude_categories' ) ) {
	function axiom_query_exclude_categories($args) {
		if (!is_admin()) {
			$ex = axiom_get_theme_option('exclude_cats');
			if (!empty($ex))
				$args['exclude'] = $ex;
...
//add_action( 'widget_posts_args', 'axiom_query_exclude_posts' );
if ( !function_exists( 'axiom_query_exclude_posts' ) ) {
	function axiom_query_exclude_posts($args) {
		if (!is_admin()) {
			$ex = axiom_get_theme_option('exclude_cats');
			if (!empty($ex)) {
				$args['category__not_in'] = explode(',', $ex);
...
if (!function_exists('axiom_query_add_sort_order')) {
	function axiom_query_add_sort_order($args, $orderby='', $order='') {
		if (empty($order)) $order = axiom_get_custom_option('blog_order');
		if (empty($orderby)) $orderby = axiom_get_custom_option('blog_sort');
		$q = array();
...
if (!function_exists('axiom_query_add_posts_and_cats')) {
...
if (!function_exists('axiom_query_add_filters')) {
	function axiom_query_add_filters($args, $filters=false) {
		if (!empty($filters)) {
				...
				if (in_array($v, array('reviews', 'thumbs'))) {							// Filter with meta_query
					...
					if (!$found) {
						...
						if ($v == 'reviews') {
							$args['meta_query'][] = array(
								'meta_filter' => $v,
								'key' => 'reviews_avg'.(axiom_get_theme_option('reviews_first')=='author' ? '' : '2'),
								'value' => 0,
								'compare' => '>',
								'type' => 'NUMERIC'
							);
					...
					$args = apply_filters('axiom_filter_query_add_filters', $args, $v);
			}
		}
...
// For example: if (axiom_check_user_role('author')) { ... }
if (!function_exists('axiom_check_user_role')) {
	function axiom_check_user_role( $role, $user_id = null ) {
...
// add_action('wp_ajax_registration_user',			'axiom_callback_registration_user');
// add_action('wp_ajax_nopriv_registration_user',	'axiom_callback_registration_user');
if ( !function_exists( 'axiom_callback_registration_user' ) ) {
	function axiom_callback_registration_user() {
		...
		} else if (($notify = axiom_get_theme_option('notify_about_new_registration'))!='no' && (($contact_email = axiom_get_theme_option('contact_email')) || ($contact_email = axiom_get_theme_option('admin_email')))) {
...
if ( !function_exists( 'axiom_allow_email_login' ) ) {
	function axiom_allow_email_login( $user, $username, $password ) {
...
/* Nav menu utils
------------------------------------------------------------------------------------- */
if ( !function_exists( 'axiom_get_nav_menu' ) ) {
	function axiom_get_nav_menu($slug='', $custom_walker=false) {
		$menu = !empty($slug) ? axiom_get_custom_option($slug) : '';
		$args = array(
			'menu'				=> empty($menu) || $menu=='default' || axiom_is_inherit_option($menu) ? '' : $menu,
			'container'			=> '',
			'container_class'	=> '',
			'container_id'		=> '',
...
if (!function_exists('axiom_require_data')) {
	function axiom_require_data( $type, $name, $args) {
...
if ( !function_exists( 'axiom_callback_clear_cache' ) ) {
	function axiom_callback_clear_cache() {
		global $_REQUEST;
...
if (!function_exists('axiom_clear_cache')) {
	function axiom_clear_cache($cc) {
