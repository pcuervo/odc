<?php
/**
 * AxiomThemes Framework: post settings
 *
 * @package	axiom
 * @since	axiom 1.0
 */
...
// Theme init
if (!function_exists('axiom_post_theme_setup')) {
	add_action( 'axiom_action_before_init_theme', 'axiom_post_theme_setup' );
	function axiom_post_theme_setup() {
		...
		if (isset($AXIOM_GLOBALS['post_meta_box']) && $AXIOM_GLOBALS['post_meta_box']['page']=='post') {
			add_action('admin_enqueue_scripts', 						'axiom_post_admin_scripts');
			add_action('axiom_action_post_before_show_meta_box',		'axiom_post_before_show_meta_box', 10, 2);
			add_action('axiom_action_post_after_show_meta_box',		'axiom_post_after_show_meta_box', 10, 2);
			add_filter('axiom_filter_post_load_custom_options',		'axiom_post_load_custom_options', 10, 3);
			add_filter('axiom_filter_post_save_custom_options',		'axiom_post_save_custom_options', 10, 3);
			add_filter('axiom_filter_post_show_custom_field_option',	'axiom_post_show_custom_field_option', 10, 4);
		}
		
		// Detect current page type, taxonomy and title
		add_filter('axiom_filter_get_blog_type',						'axiom_post_get_blog_type', 10, 2);
		add_filter('axiom_filter_get_blog_title',					'axiom_post_get_blog_title', 10, 2);
		add_filter('axiom_filter_get_current_taxonomy',				'axiom_post_get_current_taxonomy', 10, 2);
		add_filter('axiom_filter_is_taxonomy',						'axiom_post_is_taxonomy', 10, 2);
		add_filter('axiom_filter_get_stream_page_title',				'axiom_post_get_stream_page_title', 10, 2);
		add_filter('axiom_filter_get_stream_page_link',				'axiom_post_get_stream_page_link', 10, 2);
		add_filter('axiom_filter_get_stream_page_id',				'axiom_post_get_stream_page_id', 10, 2);
		add_filter('axiom_filter_get_period_links',					'axiom_post_get_period_links', 10, 3);
		add_filter('axiom_filter_detect_inheritance_key',			'axiom_post_detect_inheritance_key', 10, 1);
		add_filter('axiom_filter_list_post_types', 					'axiom_post_list_post_types', 9, 1);
		// Advanced Calendar filters
		add_filter('axiom_filter_calendar_get_month_link',			'axiom_post_calendar_get_month_link', 10, 2);
		add_filter('axiom_filter_calendar_get_prev_month',			'axiom_post_calendar_get_prev_month', 10, 2);
		add_filter('axiom_filter_calendar_get_next_month',			'axiom_post_calendar_get_next_month', 10, 2);
		add_filter('axiom_filter_calendar_get_curr_month_posts',		'axiom_post_calendar_get_curr_month_posts', 10, 2);

		// Extra column for posts/pages lists
		if (axiom_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-post_columns',			'axiom_post_add_options_column', 9);
			add_filter('manage_post_posts_custom_column',	'axiom_post_fill_options_column', 9, 2);
			add_filter('manage_edit-page_columns',			'axiom_post_add_options_column', 9);
			add_filter('manage_page_posts_custom_column',	'axiom_post_fill_options_column', 9, 2);
		}
	}
}

if ( !function_exists( 'axiom_post_settings_theme_setup2' ) ) {
	add_action( 'axiom_action_before_init_theme', 'axiom_post_settings_theme_setup2', 4 );
	function axiom_post_settings_theme_setup2() {
		axiom_add_theme_inheritance( array('archive' => array(
		...
		axiom_add_theme_inheritance( array('search' => array(
		...
		axiom_add_theme_inheritance( array('error404' => array(
			'stream_template' => '404',
			'single_template' => '',
			'taxonomy' => array(),
			'taxonomy_tags' => array(),
		...
		axiom_add_theme_inheritance( array('page' => array(
			'stream_template' => '',
		...
		axiom_add_theme_inheritance( array('post' => array(
...
if (!function_exists('axiom_post_after_theme_setup')) {
	add_action( 'axiom_action_after_init_theme', 'axiom_post_after_theme_setup' );
	function axiom_post_after_theme_setup() {
		// Update fields in the meta box
		global $AXIOM_GLOBALS;
		if (isset($AXIOM_GLOBALS['post_meta_box']) && $AXIOM_GLOBALS['post_meta_box']['page']=='post') {
			$AXIOM_GLOBALS['post_meta_box']['fields'] = array(
					...
					"info_reviews_1" => array(
						"title" => __('Reviews criterias for this post', 'axiom'),
						"override" => "post",
						"divider" => false,
						"desc" => __('In this section you can put your reviews marks', 'axiom'),
						"class" => "reviews_meta",
						"type" => "info"),
					"show_reviews" => array(
						...
						"type" => "radio",
						"style" => "horizontal",
						"options" => axiom_get_list_yesno()),
					"reviews_marks" => array(
						"title" => __('Reviews marks',  'axiom'),
						"override" => "post",
						"desc" => __("Marks for this review", 'axiom'),
						"class" => "reviews_meta reviews_tab reviews_users",
						"std" => "",
						"type" => "reviews",
						"options" => axiom_get_theme_option('reviews_criterias'))
			);
		}
	}
}
...
if (!function_exists('axiom_post_add_options_column')) {
	//add_filter('manage_edit-{post_type}_columns',	'axiom_post_add_options_column', 9);
	function axiom_post_add_options_column( $columns ){
		$columns['theme_options'] = __('Theme Options', 'axiom');
		return $columns;
	}
}

// Fill column with data
if (!function_exists('axiom_post_fill_options_column')) {
	//add_filter('manage_{post_type}_custom_column',	'axiom_post_fill_options_column', 9, 2);
	function axiom_post_fill_options_column($column_name='', $post_id=0) {
		...
		if (!empty($options)) echo '<div class="axiom_options_list">'.trim(chop($options)).'</div>';
	}
}


// Admin scripts
if (!function_exists('axiom_post_admin_scripts')) {
	//add_action('admin_enqueue_scripts', 'axiom_post_admin_scripts');
...
if (!function_exists('axiom_post_before_show_meta_box')) {
	//add_action('axiom_action_post_before_show_meta_box', 'axiom_post_before_show_meta_box', 10, 2);
	function axiom_post_before_show_meta_box($post_type, $post_id) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				...
				AXIOM_GLOBALS['reviews_levels']			= "<?php echo trim(axiom_get_theme_option('reviews_criterias_levels')); ?>";
				AXIOM_GLOBALS['reviews_max_level'] 		= <?php echo (int) $max_level; ?>;
...
if (!function_exists('axiom_post_after_show_meta_box')) {
	//add_action('axiom_action_post_after_show_meta_box', 'axiom_post_after_show_meta_box', 10, 2);
	function axiom_post_after_show_meta_box($post_type, $post_id) {
		?>
		</div>
		<?php
	}
...
if (!function_exists('axiom_post_load_custom_options')) {
	//add_filter('axiom_filter_post_load_custom_options', 'axiom_post_load_custom_options', 10, 3);
...
if (!function_exists('axiom_post_show_custom_field_option')) {
	//add_filter('axiom_filter_post_show_custom_field_option',	'axiom_post_show_custom_field_option', 10, 4);
...
if (!function_exists('axiom_post_save_custom_options')) {
	//add_filter('axiom_filter_post_save_custom_options',	'axiom_post_save_custom_options', 10, 3);
...
if ( !function_exists( 'axiom_is_post_page' ) ) {
	function axiom_is_post_page() {
		global $wp_query;
...
if ( !function_exists( 'axiom_post_list_post_types' ) ) {
	//add_filter('axiom_filter_list_post_types', 	'axiom_post_list_post_types', 9, 1);
	function axiom_post_list_post_types($list) {
		$list['post'] = __('Post', 'axiom');
		$list['page'] = __('Page', 'axiom');
		return $list;
	}
}

// Filter to detect current page inheritance key
if ( !function_exists( 'axiom_post_detect_inheritance_key' ) ) {
	//add_filter('axiom_filter_detect_inheritance_key',	'axiom_post_detect_inheritance_key', 10, 1);
	function axiom_post_detect_inheritance_key($key) {
...
if ( !function_exists( 'axiom_post_get_blog_type' ) ) {
	//add_filter('axiom_filter_get_blog_type',	'axiom_post_get_blog_type', 10, 2);
	function axiom_post_get_blog_type($page, $query=null) {
		...
		if (isset($query->queried_object) && isset($query->queried_object->post_type) && $query->queried_object->post_type=='page') {
			...
			if (axiom_substr($page, 0, 7)=='default') $page = '';
...
if ( !function_exists( 'axiom_post_get_blog_title' ) ) {
	//add_filter('axiom_filter_get_blog_title',	'axiom_post_get_blog_title', 10, 2);
	function axiom_post_get_blog_title($title, $page) {
		...
		if ( $page == 'blog' )
			...
			$title = sprintf(__('Author page: %s', 'axiom'), $curauth->display_name);
		} else if ( $page == 'error404' )
			$title = __('URL not found', 'axiom');
		else if ( $page == 'search' )
			$title = sprintf( __( 'Search Results for: %s', 'axiom' ), get_search_query() );
		else if ( $page == 'archives_day' )
			$title = sprintf( __( 'Daily Archives: %s', 'axiom' ), axiom_get_date_translations(get_the_date()) );
		else if ( $page == 'archives_month' )
			$title = sprintf( __( 'Monthly Archives: %s', 'axiom' ), axiom_get_date_translations(get_the_date( 'F Y' )) );
		else if ( $page == 'archives_year' )
			$title = sprintf( __( 'Yearly Archives: %s', 'axiom' ), get_the_date( 'Y' ) );
		 else if ( $page == 'category' )
			$title = sprintf( __( '%s', 'axiom' ), single_cat_title( '', false ) );
		else if ( $page == 'tag' )
			$title = sprintf( __( 'Tag: %s', 'axiom' ), single_tag_title( '', false ) );
		else if ( $page == 'attachment' )
			$title = sprintf( __( 'Attachment: %s', 'axiom' ), axiom_get_post_title());
		else if ( $page == 'single' )
			$title = axiom_get_post_title();
		else if ( $page == 'page' )
			$title = axiom_get_post_title();		//$title = $wp_query->post->post_title;
		else
...
if ( !function_exists( 'axiom_post_get_stream_page_title' ) ) {
	//add_filter('axiom_filter_get_stream_page_title',	'axiom_post_get_stream_page_title', 10, 2);
...
if ( !function_exists( 'axiom_post_get_stream_page_id' ) ) {
	//add_filter('axiom_filter_get_stream_page_id',	'axiom_post_get_stream_page_id', 10, 2);
	function axiom_post_get_stream_page_id($id, $page) {
...
if ( !function_exists( 'axiom_post_get_stream_page_link' ) ) {
	//add_filter('axiom_filter_get_stream_page_link',	'axiom_post_get_stream_page_link', 10, 2);
...
if ( !function_exists( 'axiom_post_get_current_taxonomy' ) ) {
	//add_filter('axiom_filter_get_current_taxonomy',	'axiom_post_get_current_taxonomy', 10, 2);
	function axiom_post_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'axiom_post_is_taxonomy' ) ) {
	//add_filter('axiom_filter_is_taxonomy',	'axiom_post_is_taxonomy', 10, 2);
	function axiom_post_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'axiom_post_get_period_links' ) ) {
	//add_filter('axiom_filter_get_period_links',	'axiom_post_get_period_links', 10, 3);
...
// Return month link
if ( !function_exists( 'axiom_post_calendar_get_month_link' ) ) {
	//add_filter('axiom_filter_calendar_get_month_link',	'axiom_post_calendar_get_month_link', 10, 2);
	function axiom_post_calendar_get_month_link($link, $opt) {
		return $link ? $link : get_month_link($opt['year'], $opt['month']);
	}
}

// Return previous month and year with published posts
if ( !function_exists( 'axiom_post_calendar_get_prev_month' ) ) {
	//add_filter('axiom_filter_calendar_get_prev_month',	'axiom_post_calendar_get_prev_month', 10, 2);
	function axiom_post_calendar_get_prev_month($prev, $opt) {
		$posts_types = array();
...
if ( !function_exists( 'axiom_post_calendar_get_next_month' ) ) {
	//add_filter('axiom_filter_calendar_get_next_month',	'axiom_post_calendar_get_next_month', 10, 2);
	function axiom_post_calendar_get_next_month($next, $opt) {
		$posts_types = array();
		if (!empty($opt['posts_types'])) {
			foreach ($opt['posts_types'] as $post_type) {
...
if ( !function_exists( 'axiom_post_calendar_get_curr_month_posts' ) ) {
	//add_filter('axiom_filter_calendar_get_curr_month_posts',	'axiom_post_calendar_get_curr_month_posts', 10, 2);
	function axiom_post_calendar_get_curr_month_posts($posts, $opt) {
		$posts_types = array();
		if (!empty($opt['posts_types'])) {
			foreach ($opt['posts_types'] as $post_type) {
