...
// Theme init
if (!function_exists('axiom_po_composer_theme_setup')) {
	add_action( 'axiom_action_after_init_theme', 'axiom_po_composer_theme_setup' );		// Fire this action after load theme options
...
class axiom_po_composer {
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			axiom_enqueue_style('po-composer-style', axiom_get_file_url('tools/po_composer/po_composer.css', array(), null));
		}
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			...
			axiom_enqueue_script('po-composer-script', axiom_get_file_url('tools/po_composer/po_composer.js', array('jquery'), null, true));
		}
	}
	...
	function build_page() {
		...
		$options  =  "\n".'<option value="upload_">'.__('Upload .po-file', 'axiom').'</option>'
					."\n".'<option value="edit_"'.(in_array($this->po_src, array('upload_', 'edit_')) ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'axiom').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'axiom') : __('Languages', 'axiom')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->po_src))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'axiom').'">'.trim($this->get_list_options($child_list, 'child_', $this->po_src))."\n".'</optgroup>' : '');
		$options2 = "\n".'<option value="">'.__('- Select merge source -', 'axiom').'</option>'
					."\n".'<option value="upload_"'.($this->error && $this->po_src2=='upload_' ? ' selected="selected"' : '').'>'.__('Upload .po-file', 'axiom').'</option>'
					."\n".'<option value="edit_"'.($this->error && $this->po_src2=='edit_' ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'axiom').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'axiom') : __('Languages', 'axiom')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'axiom').'">'.trim($this->get_list_options($child_list, 'child_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>' : '');

		$nonce = wp_create_nonce(__FILE__);
		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php _e('AxiomThemes .PO-files Composer', 'axiom'); ?></h2>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Edit .po-file:', 'axiom'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src"><?php _e('Select .po-file:', 'axiom'); ?></label>
							...
							<div class="po_composer_field po_composer_editor">
								...
								<input type="checkbox" value="1" name="po_translated_down" id="po_translated_down"<?php echo isset($_POST['po_translated_down']) ? ' checked="checked"' : ''; ?> /><label for="po_translated_down"><?php _e('Move translated strings down', 'axiom'); ?></label>
								</span>
								<ul class="right_side">
									<li id="po_composer_link_text" class="po_composer_link_text"><a href="#po_composer_editor_text"><?php _e('Plain text', 'axiom'); ?></a></li><li id="po_composer_link_list" class="po_composer_link_list"><a href="#po_composer_editor_list"><?php _e('Strings editor', 'axiom'); ?></a></li>
								</ul>
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Merge with .po-file:', 'axiom'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src2"><?php _e('Select .po-file:', 'axiom'); ?></label>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo ($this->po_src=='' ? __('Upload', 'axiom') : __('Update', 'axiom')); ?></a>
					<?php if (!empty($this->po_link) || !empty($this->mo_link)) { ?>
					<div class="po_composer_links">
						<?php if (!empty($this->po_link)) { ?>
							<a href="<?php echo esc_url($this->po_link); ?>" class="po_composer_download_link"><?php _e('Download .PO-file', 'axiom'); ?></a>
						<?php } ?>
						<?php if (!empty($this->mo_link)) { ?>
							<a href="<?php echo esc_url($this->mo_link); ?>" class="po_composer_download_link"><?php _e('Download .MO-file', 'axiom'); ?></a>
						<?php } ?>
					</div>
					<?php } ?>
				</div>
