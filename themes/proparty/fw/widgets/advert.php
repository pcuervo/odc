 ...
 */
add_action( 'widgets_init', 'axiom_widget_advert_load' );

/**
 * Register our widget.
 */
function axiom_widget_advert_load() {
	register_widget( 'axiom_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class axiom_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => __('Advertisement block', 'axiom') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'axiom_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'axiom_widget_advert', __('AxiomThemes - Advertisement block', 'axiom'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Advertisement block', 'axiom') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php _e('Image source URL:<br />(leave empty if you paste advert code)', 'axiom'); ?></label>
            ...
            <?php
			echo trim(axiom_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php _e('Image link URL:<br />(leave empty if you paste advert code)', 'axiom'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php _e('or paste Advert Widget HTML Code:', 'axiom'); ?></label>
